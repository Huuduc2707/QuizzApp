...
if (!isset($_SESSION['username'])) {
  ...
  $id = $_GET['id'];
  $sql = "SELECT e.Date, bhg.Quantity, bhg.Total_cost, c.First_name Customer_first, c.Last_name Customer_last, em.First_name Cashier_first, em.Last_name Cashier_last, g.Name Goods_name, g.Price, g.Type, s.Name
          FROM export_bill e JOIN bill_has_goods bhg ON e.ID = bhg.Export_bill_ID 
          JOIN customer c ON e.Customer_ID = c.ID 
          JOIN employee em ON em.ID = e.Cashier_ID 
          JOIN saving_point_policy svp ON svp.ID = e.Saving_point_policy_ID  
          JOIN goods g ON g.ID = bhg.Goods_ID  
          JOIN supermarket s ON s.SCode = em.Supermarket_Scode 
          WHERE bhg.Export_bill_ID = $id";
  $detailsArray = $conn->query($sql)->fetch_all(MYSQLI_ASSOC);
?>

  <div id="main-content">
    <div class="page-heading">
      <div class="page-title mb-2">
        <h1 style="display:inline" class="me-4">Details</h1>
          ...
          <div class="card h-100 mb-4">
            ...
            <div class="card-body" style="width:100%">
              <table class="table table-hover datatable">
                ...
                <tbody>
                  <?php foreach ($detailsArray as $em) {
                  ?>
                    <tr>
                      <td><?= $em['Date'] ?></td>
                      <td><?= $em['Name'] ?></td>
